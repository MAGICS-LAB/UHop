...
def get_args():
    ...
    parser.add_argument('--kernel_epoch', type=int, default=50)
...
class Exp:
    def __init__(self, args):
        ...
        self.model = ViH(in_channels=self.args.channel, patch_size=args.patch_size, emb_size=args.d_model, img_size=args.img_size*args.img_size, n_heads=args.n_heads, mode=args.mode, n_class=args.n_class).cuda() # embedding size P^2*C
...
def main():
    ...
    plt.savefig(f'imgs/train_loss_{args.data}_{args.datasize}.png')
    plt.clf()

    sns.lineplot(data=plot_data, x="epoch", y="train acc", hue="mode")
    plt.savefig(f'imgs/train_acc_{args.data}_{args.datasize}.png')
    plt.clf()

    sns.lineplot(data=plot_data, x="epoch", y="test loss", hue="mode")
    plt.savefig(f'imgs/test_loss_{args.data}_{args.datasize}.png')
    plt.clf()

    sns.lineplot(data=plot_data, x="epoch", y="test acc", hue="mode")
    plt.savefig(f'imgs/test_acc_{args.data}_{args.datasize}.png')
    plt.clf()

    df = pd.DataFrame(plot_data)
    df.to_csv(f"results/{args.data}_result_warmup_{args.datasize}.csv")
