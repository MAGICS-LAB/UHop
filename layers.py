...
from functions import softmax_1
...
class LearnableHopfield(nn.Module):
    ...
    def __init__(
            ...
            kernel='lin',
            scale=None):
        super(LearnableHopfield, self).__init__()

        d_keys = d_keys or (d_model // n_heads)
        d_values = d_values or (d_model // n_heads)
        self.d_model = d_model
        if kernel == 'lin':
            self.kernel = nn.Linear(d_model, d_model)
        elif kernel == 'icnn':
            pass
        self.inner_attention = Association(
            scale=scale, attention_dropout=dropout, mode=mode)
    ...
    def uniform_forward(self, Y):
        # this is used to train the kernel
    ...
    def forward(self, R, Y, mask=None):
        ...
        queries = self.query_projection(self.kernel(R)).view(B, L, H, -1)
        keys = self.key_projection(self.kernel(Y))
        values = self.value_projection(Y).view(B, S, H, -1)
        keys = keys.view(B, S, H, -1)
